...
const withSearchContentNoResult = (Component) => (props) => (Object.keys(props.search).length === 0 || props.search.filter.length < 1
  ?	<div className="home-content-search">{`No results found for "${props.search.input}"`}</div>
  :	<Component {...props} />);

const SearchContentFiltered = ({
  search, jobs, onSetJobs, firebase, authUser, isJobsActive, onSetIsJobsActive, onSetFiles, onSetSearch,
}) => {
  ...
  const updateSearch = (newFiles) => {
    const { input } = search;
    const library = [...newFiles, ...connections];
...
const SearchContent = ({
  search, firebase, authUser, jobs, onSetJobs, isJobsActive, onSetIsJobsActive, onSetFiles, onSetSearch,
}) => (
