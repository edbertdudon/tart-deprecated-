...
import { getFormulaColors } from '../../functions';
import './index.less';
...
const Formulabar = ({ slides, formula }) => {
  ...
  const formulaColors = getFormulaColors(cell).map((c) => (
    <span style={{ color: 'color' in c && c.color }} id={c.id}>
      {c.text}
    </span>
  ));

  return (
    <div className="formulabar" onInput={handleChange} ref={inputRef} contentEditable>
      {formulaColors}
    </div>
  );
};
